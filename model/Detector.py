...
class Detector(object):
    ...
    def __init__(self, host="localhost", ports={"pnet": "8503", "rnet": "8504", "onet": "8505"}):
    ...
    def detect_face_12net(self, cls_prob, roi, out_side, scale, width, height, threshold):
        ...
        mask = tf.where(tf.math.greater(cls_prob, threshold), tf.ones_like(cls_prob),
                        tf.zeros_like(cls_prob))  # mask.shape = (h,w)
        ...
        boundingbox = tf.math.round(
            (stride * tf.tile(pos, (1, 2)) + tf.constant([0, 0, 11, 11], dtype=tf.float32)) * scale)
    ...
    def filter_face_24net(self, cls_prob, roi, rectangles, width, height, threshold):
        ...
        mask = tf.where(tf.math.greater(cls_prob, threshold), tf.ones_like(cls_prob),
                        tf.zeros_like(cls_prob))  # mask.shape = (target num,)
        mask = tf.cast(mask, dtype=tf.bool)
    ...
    def detect(self, img, threshold=None):
        ...
        def refine_rectangle(rect):
            points = [int(u.numpy()) for u in rect[:4]]
            points[2] -= points[0]
            points[3] -= points[1]
            return {'face': points}
        ...
        for scale in scales:
            ...
            requests_data = json.dumps({"signature_name": "serving_default",
                                        "instances": [{"input_4": np.squeeze(resized_img.numpy()).tolist()}]})
            json_response = requests.post("http://" + self.host + ":" + self.ports["pnet"] + "/v1/models/pnet:predict",
                                          data=requests_data, headers=self.headers)
        ...
        # 2) crop and refine
        boxes = tf.stack([rectangles[..., 1], rectangles[..., 0], rectangles[..., 3], rectangles[..., 2]],
                         axis=-1) / tf.constant([h, w, h, w], dtype=tf.float32)
        predict_24_batch = tf.image.crop_and_resize(inputs, boxes, tf.zeros((rectangles.shape[0],), dtype=tf.int32),
                                                    (24, 24))
        cls_prob, offset = [], []
        for p in predict_24_batch:
            # send request
            requests_data = json.dumps(
                {"signature_name": "serving_default", "instances": [{"input_3": np.squeeze(p.numpy()).tolist()}]})
            json_response = requests.post("http://" + self.host + ":" + self.ports["rnet"] + "/v1/models/rnet:predict",
                                          data=requests_data, headers=self.headers)
            batch_response = json.loads(json_response.text)["predictions"][0]
            cls_prob.append(tf.constant(batch_response["conv5-1"], dtype=tf.float32))
            offset.append(tf.constant(batch_response["conv5-2"], dtype=tf.float32))
        outs = (np.array(cls_prob), np.array(offset))
        ...
        boxes = tf.stack([rectangles[..., 1], rectangles[..., 0], rectangles[..., 3], rectangles[..., 2]],
                         axis=-1) / tf.constant([h, w, h, w], dtype=tf.float32)
        predict_batch = tf.image.crop_and_resize(inputs, boxes, tf.zeros((rectangles.shape[0],), dtype=tf.int32),
                                                 (48, 48))
        cls_prob, offset, pts = [], [], []
        for p in predict_batch:
            # send request
            requests_data = json.dumps(
                {"signature_name": "serving_default", "instances": [{"input_2": np.squeeze(p.numpy()).tolist()}]})
            json_response = requests.post("http://" + self.host + ":" + self.ports["onet"] + "/v1/models/onet:predict",
                                          data=requests_data, headers=self.headers)
            batch_response = json.loads(json_response.text)["predictions"][0]
            cls_prob.append(tf.constant(batch_response["conv6-1"], dtype=tf.float32))
            offset.append(tf.constant(batch_response["conv6-2"], dtype=tf.float32))
            pts.append(tf.constant(batch_response["conv6-3"], dtype=tf.float32))

        outs = (np.array(cls_prob), np.array(offset), np.array(pts))
        cls_prob = outs[0][..., 1]
        offset = outs[1]
        pts = outs[2]
        rectangles = self.filter_face_48net(cls_prob, offset, pts, rectangles, w, h, threshold[2])

        return [refine_rectangle(r) for r in rectangles]
